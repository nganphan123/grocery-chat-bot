...
def delivery_time_estimate(params):
    ...
    '''
    # Parse the address
    address = str(params["street-address"]) + " " + str(params["geo-city"]) + " " + str(params["geo-state"]) + " " + str(params["geo-country"])
    address.replace(",", " ")
    address = re.sub(r'\s+', '%20', address)

    params = {'key' : os.getenv('API_KEY'), 'origins' : '1555%20Banks%20Rd%20Kelowna%20BC%20Canada', 'destinations' : address, 'departure_time': 'now'}
